...
let ctx = null
let particles = []
const mouse = { x: globalThis?.innerWidth / 2, y: globalThis?.innerHeight / 2 }
const targetMouse = { x: globalThis?.innerWidth / 2, y: globalThis?.innerHeight / 2 }
const lastMouse = { x: globalThis?.innerWidth / 2, y: globalThis?.innerHeight / 2 }
let animationFrameId = null
...
function updateMousePosition() {
  ...
  const mouseSpeed = Math.sqrt(
    (targetMouse.x - lastMouse.x) ** 2 + (targetMouse.y - lastMouse.y) ** 2
  )
